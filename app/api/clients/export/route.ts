...
export async function GET(req: NextRequest) {
  try {
    ...
    /* ================= QUERY PARAMS ================= */
    // ✅ Fix #2: Use req.nextUrl instead of new URL(req.url)
    ...
    if (type === "today") {
      ...
      where.visitingDate = { gte: start, lt: end };
    }

    // ✅ Fix #3: Apply active filters when type=filtered
    if (type === "filtered") {
    ...
    clients.forEach((client) => {
      worksheet.addRow({
        ...
        visitingDate:      client.visitingDate  ? client.visitingDate.toLocaleDateString()  : "",
        followUpDate:      client.followUpDate  ? client.followUpDate.toLocaleDateString()  : "",
